...
for p=1:length(pldaps_filenames)
    ...
    % pldaps iTrial is reset ([Inf iTrial], rather than iTrial==1, allows for first pldaps to start
    % after Trellis recording, although this should be avoided if possible!)
...
%     firstTrial(p) = find( unique_trial_number(:,4)>=str2double(hhmm(1:2)) & unique_trial_number(:,5)>=str2double(hhmm(3:4)) & iTrial==1,1);
    ...
    if str2double(filedate)<=20220504
        parNum(firstTrial(p):lastTrial(p)) = find(strcmp(parOptions,paradigms(p)));
    end
...
% add nr ? 2022-05
hdr.infolabels = {'headingInd','modality','coherenceInd','deltaInd','choice','correct','PDW','numDirs','nr','aperture'};
        ...
        if str2double(filedate)<=20220301 
            ...
            if str2double(filedate)>=20220610
                if length(tr_info)==numDirs+4
                    ...
                    nsEvents.Events.nr(t) = tr_info(end-1);
                    nsEvents.Events.apertureDiam(t) = tr_info(end-2);
                    tr_info(1:end-4) = []; 
...
%     nsEvents.pldaps.iTrial(t) = tr_events(7)-(2^9+2^8);
    ...
    % when Nexonar is on, bit vals will be offset. account for this and re-assign to tr_events
    %
...
%     end
    ...
    % for RFMapping, this will be the very first stimOn and final stimOff
    ...
    if strcmp(nsEvents.pldaps.parName{t},'RFMapping') % should be numDirs*2 motion events
        try 
...
% changed from reward, because reward time will be NaN on error trials, we
% want 'good trials' just to denote non-brfix (i.e. completed) trials
