...
close all
            ...
            % make the cohs almost the same, but different, to still use
            % the larger one as dummy var for some manip (eg TMS)
            data_orig = data;
            % cohs = unique(data.coherence);
            % data.coherence(data.coherence==cohs(2)) = cohs(1)+0.001;
            
            data.coherence(data.coherence==0.4) = 0.6;
            data.coherence(data.coherence==0.7) = 0.601;
