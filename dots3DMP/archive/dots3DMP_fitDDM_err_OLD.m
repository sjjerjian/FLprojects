function [err,data] = dots3DMP_fitDDM_err(param, data, options)
...
% % %     end
    ...
    Ptb_coh = zeros(max_dur, 2, length(strength_set));                % time * bound * signed strength
    Ptb_coh_forMarginals = Ptb_coh;
    % probability density of decision variable (x) across time
    Pxt_coh = zeros(length(xmesh), max_dur, length(strength_set));    % xmesh * time * signed strength
    ...
    Pxt_marginal = zeros(length(xmesh), max_dur, 2);                % xmesh * time * motion_direction
     ...
     %the map is usually thought of as a product of experience with the
     %conditions presented in an experiment. thus, although we have
     %different k values for each condition, and maybe even different
     %bounds, variances, etc., we can still compute a single map, based on
     %the aggregate or average relationship between accumulated evidence
     %and pct correct. This aggregation should respect the various
     %frequencies of the different conditions, and it's challenging to
     %compute it for combined condition. So for simplicity let's just
     %average ves, vislow, vishigh.
     
     cohs = unique(data.coherence);
     wVisLow = sqrt( (kvis*cohs(1))^2 / ((kvis*cohs(1))^2 + kves^2) );
     wVesLow = sqrt( kves^2 / ((kvis*cohs(1))^2 + kves^2) );
     wVisHigh = sqrt( (kvis*cohs(2))^2 / ((kvis*cohs(2))^2 + kves^2) );
     wVesHigh = sqrt( kves^2 / ((kvis*cohs(2))^2 + kves^2) );
...
% % %         F = strength_set_freq(strength_set_unsigned==abs(strength_set(s))); % MVL
     ...
     %*** aaaand here's where we hit the brick wall.
         
     % can't generate probabilities of each outcome when confidence is
     % continuous. therefore can't calculate the likelihood of the data
     % given the parameters, unless we take the RT approach and have a
     % distribution over values of confidence. Okay maybe it's doable, but
     % not in a reasonable amount of time.
     % Could try to basically run the simulation for each trial, in
     % each iteration of the params, and minimize the sse.. but I bet
     % that's too slow to be practical.
