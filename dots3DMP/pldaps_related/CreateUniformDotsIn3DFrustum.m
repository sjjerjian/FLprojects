function [x,y,z] = CreateUniformDotsIn3DFrustum(ndots,FOV,aspectr,depthrangen,depthrangef,eyeheight,stream)
...
% 2022-01-05 SJ  Added stream for RandStream support

psychassert(all(depthrangen<=depthrangef),'Near clipping plane should be closer than far clipping plane');

u   = RandLim([1,ndots],0,1,stream);                                           % get uniform random variable

if nargin>5 && ~isempty(eyeheight)
    ...
    % cloud
    z   = -(u.*(depthrangef.^3-depthrangen.^3)+depthrangen.^3).^(1/3);      % transform to parabolar distribution (negate as depth position is a negative number
end

yrs = -z*tand(FOV/2);
if nargin>5 && ~isempty(eyeheight)
    y   = -eyeheight*ones(1,ndots);
%     y  = -eyeheight.*u.^2.*-eyeheight;
else
    y   = RandLim([1,ndots],-yrs,yrs,stream);
end
xrs = yrs * aspectr;
x   = RandLim([1,ndots],-xrs,xrs,stream);

% for a uniform sample over z:
% z   = -RandLim([1,ndots],depthrangen,depthrangef,stream);
