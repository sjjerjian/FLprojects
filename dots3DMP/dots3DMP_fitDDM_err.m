...
% what we want is a MLE but we don't have an analytical form for the
% likelihood

% the likelihood is P(data|params)
% where data is a set of Ntrials w three observations: choice, RT, and conf

% what is the distribution P(choice|params)? can it be anything other than binomial?

% RT dists are tough to deal with even in the best of circumstances;
% here it has a funny peak out at the max (when bound not hit)

% the distribution of confidence? also weird shaped because of bound

% you might think that the distributions from the simulation are what
% matters, but really what you need is the probability (likelihood) of a
% given trial (type) having the outcome that the simulation gives. So for a
% monte-carlo approach you'd want to simulate many repeats of a particular
% coh, delta, mod, etc -- remember what Luigi says: think about the
% stochastic process that generates the data!
