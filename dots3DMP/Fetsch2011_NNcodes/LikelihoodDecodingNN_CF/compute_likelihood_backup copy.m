...
function [r,L] = compute_likelihood(heading,resp_trial_all,resp_mean_all,s,j,k,i,use_logL,param1_interp,param2_interp,likemodel)
...
for n = 1:N
    ...
    n_shuf = randperm(length(resp_trial_all{n}{s,j,k,i}));
    if strcmp(likemodel,'Poisson')
        r(n) = round(resp_trial_all{n}{s,j,k,i}(n_shuf(1)));
    else
        r(n) = resp_trial_all{n}{s,j,k,i}(n_shuf(1)); % no need to round for histogram or fitted likelihood! (in fact it screws it up)
    end
...
%-------------------------------------------------------------
    ...
    %             disp(['Inf problem after scale-down! Cell ID #' num2str(n)]);
                f(n,:) = f(n,:) * 0.5; % This kluges one cell whose high FR makes the equation exceed
                r(n) = round(r(n)*0.5); % the largest representable positive floating point number
            end
            if sum((g*f(n,:)).^r(n)) == Inf || factorial(r(n)) == Inf
...
%                 %%%ABCD%%%%
%                 figure(21); clf; plot(heading,like); title(num2str(n)); pause;
%                 %%%ABCD%%%%
...
if use_logL && strcmp(likemodel,'Poisson')
    % L = sum(term1) - sum(f) - sum(term3);
    L = sum(term1) - sum(f);  % term 2 is just the sum of the f's, and term 3 is independent of hdg
else
    if N>1
        L = prod(temp_L);
    else
...
% L = L/sum(L);
% figure(1); clf; plot(hdg,L);
% title([num2str( sum(L(1:find(hdg==0)-1))) '   ' num2str(sum(L(find(hdg==0)+1:end)))]);
% pause;
% %%%ABCD%%%%
                
% if sum(L)==0 || isnan(sum(L))
%     asdf;
% end

% if s==2 && k==1
%     figure(1232); set(gcf,'Position',[50,600,1500,300]); hold on;
%     subplot(1,7,i); plot(hdg,L/sum(L),'b',hdg,L2/sum(L2),'r'); title(num2str(heading(i))); hold on;
% %     title(['s j k i hdg: ' num2str([s j k i heading(i)])]); 
%     if i==1
%         legend('Poisson','nonPoisson');
%     end
% end
