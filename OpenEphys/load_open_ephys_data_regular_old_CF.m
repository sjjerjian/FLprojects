function [data, timestamps, info, events] = load_open_ephys_data(filename, CFevents, varargin)
...
%------------------------- EVENT DATA ----------------------------------
%-----------------------------------------------------------------------
if (strcmp(filetype, 'events') && ~exist('CFevents', 'var')) || strcmp(filetype, 'events') && CFevents == 0
...
%     excludes = {'Processor','Software','IsRecording'};
    ...
    for t = 1:T-1 % loop over trials
        ...
        for n = trInd(t):trInd(t+1)-1 % for each trial, loop over events (all events within each 'TrialStart')
            ...
            elseif ismember(thisStr,trialParams) %mvl: whatever isnt fill is left off as NaN, which is convinient
                val = str2double(edata{n}(sp(2)+1:end-1));
                eval(['events.' thisStr '(t)=val;']);
            elseif ismember(thisStr,both)
                if ismember(thisStr,'ApertureX')
                    %ts = str2double(edata{n}(1:sp(1)-1));
                    val = str2double(edata{n}(sp(2)+1:end-1));
                    %eval(['events.' thisStr '{t}(1,end+1)=ts;']);
                    events.ApertureX{t}(end+1) = val;
                elseif ismember(thisStr,'ApertureY')
                    %ts = str2double(edata{n}(1:sp(1)-1));
                    val = str2double(edata{n}(sp(2)+1:end-1));
                    %eval(['events.' thisStr '{t}(1,end+1)=ts;']);
...
elseif strcmp(filetype, 'continuous')
    ...
    if isempty(range_pts)
        ...
        while ftell(fid) + RECORD_SIZE <= filesize % at least one record remains
            ...
            if nsamples ~= SAMPLES_PER_RECORD && version_ >= 0.1
    ...
    elseif ~isempty(range_pts)
        ...
        if any(info.nsamples ~= SAMPLES_PER_RECORD) && version_ >= 0.1
            disp(['  Found corrupted record...searching for record marker.']);
...
if (isfield(info,'header'))
    if (isfield(info.header,'sampleRate')) %seems to always be 30,000
...
if ~exist('events','var')
    events = NaN; % CF, possibly temp: must assign all output vars
end
