...
% SJ wrote this sometime in mid-2023, to simplify the PLDAPS_preprocessing
% process. 
...
% 2. on windows PC with mounted NAS (or anywhere really), downloading and re-saving cleaned
% versions of each PDS file is a huge time sink. It's relatively fast to
% just read raw PDS files directly from the mounted drive, and create the
% datastruct here and now. Obviously downloading and saving the PDS files
% locally means you don't have to re-download these files each time, but
% having them stored on a local computer can also eat space quickly.
...
opts = struct('eyemovement', save_eyemov, 'reward', save_reward);
...
function [data] = pds_preprocessing_dots3DMP(subject, paradigm, dateRange, localDir, data, options)
    % main function to run the pds_preprocessing without saving individual files to local
    % final dataStruct will be saved in user-specified location
    %
    % specifying an existing data struct should allow for appending,
    ...
    % if saving eye mov or nexonar signals, event times are useful too!
    ...
    elseif strcmp(hostname, 'DESKTOP-JRJ0F9N')
        ...
        error('remoteDir and mountDir not yet defined for Windows')
    end
                    ...
                    if isfield(PDS.data{t}.behavior,'choice') % excluding trials with missing data
                        ...
                        if options.eyemovement
                            try
                                ...
                                % this way we can shift eye signals on each
                                % trial so that they roughly all line up to
                                % the same period (time 0 will be the align_event)
...
%                                 zero_pos = find(xdata>0.1, 1) + zero_pos + eyewin_samples(1);
                        ...
                        if options.dotposition
                            % TODO modify to store dotPos as data x trials
                            % array, like eye_mov, otherwise this will
                            % balloon file sizes
                            try
