...
load('VarsFor_DDMDecode_PDW_YYY.mat')
% load stimMT_LOmap_YYY.mat
...
DV = nan(nTrials,length(tAxis)); % store vector of dv values for each trial through all time, 'ms'

sigmaPooling = 3;
% I = abs(coh)> 0.01;
% k = median(Mu(I)./coh(I)); % "median seems to work fine"
k = 2.328125;
B = 105;       % Probly best to bring this in via VarsFor_DecodeDDM_PDW file.
% sigma = mean(Sigma);
sigma = 3.8763;
% sigma = 4.13728378493874;
theta = 1.2; % threshold (in log odds), for high bet (% hand tuned to get reasonable PDW)
...
for tr = 1:nTrials
    ...
    poolingNoise = round(sigmaPooling*randn(1,trTime)); % Vector of noise values of length(trTime)
    %     rightspikes(dur(tr)+latency+1:end) = NaN; % Start vector after latency then fill times beyond RT with Nans
    %     leftspikes(dur(tr)+latency+1:end) = NaN; % If want to use uncomment

        if tr == 1   %  Only want to run these operations one time total
            % Can load logOdds maps throuh loadVar above so don't have to
            % run at all. These are stable value look up tables.
    %
...
%             logOddsCorrMap_mean = mean(logOddsCorrMap,1,'omitnan'); % Avg logOdds value at each 'ms'
            % max(logOdds,1) = degrades performance drastically, mostly noticeable in low conf choice, RTs are slower, < .5 p(Hi bet) at 0% coh
            % min() = "strong feedback"
%             lambda_mod = nan(nTrials,trTime);
    ...
    for ms = 1:dT:trTime % At each ms timepoint compare 'rnd' to each neurons spike probability. If the values are close in value spike occurs. Define **close**: In simMT_build difference between rnd and cspf value is minimized. Capture this minimized value for each timepoint.
        ...
        %         if ms == 1 % Run once for each trial
        %             Ry(tr,:,ms) = poissrnd(lambda); % Generate spike or no spike for time ms ==1
        %             DV(tr,ms) = sum(Ry(tr,rightpool,ms),'omitnan') - sum(Ry(tr,leftpool,ms),'omitnan') + poolingNoise(ms);
        %
        %             %             rightspikes(dur(tr)+latency+1:end) = NaN; % Start vector after latency then fill times beyond RT with Nans
        %             %             leftspikes(dur(tr)+latency+1:end) = NaN;
        %
        %             %             rightspikes(ms) = sum(Ry(tr,rightpool,ms),3,'omitnan'); % Current spike count total for each pool, counts through till time ms
        %             %             leftspikes(ms) = sum(Ry(tr,leftpool,ms),3,'omitnan'); % Try without 'omitnan' so length matches up
        %             %             DV(tr,ms) = rightspikes(ms) - leftspikes(ms) + poolingNoise(ms);
        %
        %         else % i.e., on all trials when ms>1
        %
        %                     if DV(tr,ms-1)>0 % Rightward favored
        %                         lambdaLoser = lambda; % placeholder operation; % Lambda = some function of previous "OG" lambda, modulated proportionally to LO ad current DV                           Old + new, what operation modulates lambda appropriately relative to logOdds value?
        %                         lambdaWinner = lambda; % placeholder operation % lambda is a vector of 360 values
        %
        %                         lambda(rightpool) = lambdaWinner(rightpool); % Rightpool uses winner
        %                         lambda(leftpool) = lambdaLoser(leftpool);
        %                         Ry(tr,:,ms) = poissonrnd(lambda);
        %                     elseif DV(tr,ms-1) < 0 % i.e., R is not greater L, i.e., DV < 0
        ...
        %
        %                         lambda(leftpool) = lambdaWinner(leftpool); % leftpool uses Winner
        %                         lambda(rightpool) = lambdaLoser(rightpool);
        %                         Ry(tr,:,ms) = poissrnd(lambda);
        %                     end
        if ms == 1
            Ry(tr,:,ms) = poissrnd(lambda); % Generate spike or no spike for time ms==1
            DV(tr,ms) = abs(sum(Ry(tr,rightpool,ms),'omitnan')) - abs(sum(Ry(tr,leftpool,ms),'omitnan')) + poolingNoise(ms);
        else
%             lambda_mod(tr,ms) = logOddsCorr_current / logOddsCorrMap_mean(ms-1); % This lambda mod requires a theta < 1.5, 1.2 gave p(hi) of .53 at 0% coh can leave it at that or try 1.25
            % Define lambda for L and Rpool respectively, as some function of logOdds at ms-1
%             if DV(tr,ms-1)>0 % Rightward favored
% 
%                 lambdaLoser = lambda; % placeholder operation; % Lambda = some function of previous "OG" lambda, modulated proportionally to LO ad current DV                           Old + new, what operation modulates lambda appropriately relative to logOdds value?
%                 lambdaWinner = lambda; % placeholder operation % lambda is a vector of 360 values
...
%                 lambda(leftpool) = lambdaLoser(leftpool);
% 
%                 Ry(tr,:,ms) = poissrnd(lambda);% Spike no Spike?
% 
%             elseif DV(tr,ms-1) < 0 % i.e., R is not greater L, i.e., DV < 0
%                 lambdaLoser = lambda; % *probR placeholder operation; % Old + new what operation modulates lambda appropriately relative to logOdds value
%                 lambdaWinner = lambda; % *probL placeholder operation
...
%                 Ry(tr,:,ms) = poissrnd(lambda); % Spike no Spike?
%             end
%             Ry(tr,:,ms) = poissrnd(lambda*lambda_mod(tr,ms)); % Spike no Spike?
            Ry(tr,:,ms) = poissrnd(lambda);
            DV(tr,ms) = sum(Ry(tr,rightpool,ms),'omitnan') - sum(Ry(tr,leftpool,ms),'omitnan') + DV(tr,ms-dT) + poolingNoise(ms); % R - L spikes at current 'ms' + this diff at last 'ms'
        end

        if abs(DV(tr,ms))>B
            hitBound(tr) = 1;
            choice(tr) = sign(DV(tr,ms)); % sign give 1 or -1 depending if value > or < 0. If value is 0 then sign gives 0
            RT(tr) = ms;
            finalV(tr) = B*sign(DV(tr,ms)); % cap bound crossings at bound value, neg or positive bound crossed? Store variable

            % log odds correct is the value at the bound (not past it)
            DV_atB = sign(DV(tr,ms))*B; % Same as finalV but not stored, redefined on each trial for in loop use
            whichV = find(abs(vAxis-DV_atB)==min(abs(vAxis-DV_atB))); % Using DV_atB.. all decisions are made at same DV value, 'B', thus same logOdds value across all trials and time if DV >= B
            whichT = find(abs(tAxis-ms)==min(abs(tAxis-ms)));
            logOddsCorr(tr) = logOddsCorrMap(whichV(1), whichT(1)); % Index to first position just in case min() outputs more than 1 value
            pdw(tr) = logOddsCorr(tr) >= theta; % Hi or Lo wager / 1 or 0
        ...
        elseif ms==trTime % out of time, bound not hit
            ...
            hitBound(tr) = 0;
            choice(tr) = sign(DV(tr,ms));
            RT(tr) = ms;
            finalV(tr) = B*sign(DV(tr,ms)); % cap bound crossings at bound value, neg or positive bound crossed? Store variable
            
            whichV = find(abs(vAxis-DV(tr,ms))==min(abs(vAxis-DV(tr,ms)))); % Find index to vAxis value closest to DV(tr,ms) value
            whichT = find(abs(tAxis-ms)==min(abs(tAxis-ms)));
            logOddsCorr(tr) = logOddsCorrMap(whichV(1), whichT(1));
            pdw(tr) = logOddsCorr(tr) >= theta; % Hi or Lo wager, 1 / 0

        else % otherwise continue accumulating, i.e., if not yet hit DV bound and still time in trial..
            % map current DV to log odds
            whichV = find(abs(vAxis-DV(tr,ms))==min(abs(vAxis-DV(tr,ms))));
            whichT = find(abs(tAxis-ms)==min(abs(tAxis-ms)));
            logOddsCorr_current = logOddsCorrMap(whichV(1), whichT(1)); % Index to first position just in case min() has multiple outputs
        end
        ...
        % logOddsMapR/L.
        %
        %         dv_lookup = find(min(abs(vAxis-DV(tr,ms)))); % Find index to vAxis value closest to current dv (at time 'ms').
        %         % Can use logOddsCorrMap instead of L v R, and just ask if DV is
        %         % neg or positive
...
% Make sure to initiate simMT_test with nNeuron and nTrial values in this script

% disp('saving...');
% % 
% clearvars -except sigma B k CP_noFB pdw choice RT leftpool rightpool Ry tuning nNeurons nTrials prefDirs latency dir dur coh cohs poscohs tAxis
% % 
% tic
% save(sprintf('simMT_wNoFB_nNeu=%d_nTr=%d.mat', nNeurons, nTrials),'-v7.3');
% toc
% % 
% disp('file saved.');


%% Choice probability (Britten et al. 1996)
% clear;
...
spCount = squeeze(sum(Ry,3,'omitnan')); % Collapse Ry into 2d matrix storing each neurons total trial spCount

ck = find(rightpool);
CP = nan(size(ck'));
for g = 1:length(find(rightpool))
    CP(g) = rocN(spCount(chooseright,ck(g)), spCount(chooseleft,ck(g)), 50);
end
meanCP = mean(CP)
CP_right = CP;
...
ck = find(leftpool);
CP = nan(size(ck'));
for g = 1:length(find(leftpool))
    CP(g) = rocN(spCount(chooseleft,ck(g)), spCount(chooseright,ck(g)), 50);
end
meanCP = mean(CP)
CP_left = CP;

mean([CP_left; CP_right])
allCPs = [CP_left; CP_right];
% 
% histogram(CP_left,'FaceColor','b','FaceAlpha',.45); hold on;
% histogram(CP_right,'FaceAlpha',.7,'FaceColor','b'); xline([mean(allCPs) mean(CP_left) mean(CP_right)],'r--',{'.5396','.5229','.5563'}, 'LineWidth',1); 
% title('CP distribution: No Feedback'); changeAxesFontSize(gca,18,18); legend('Leftpool','Rightpool'); 
% xlabel('Choice Probability'); ylabel('# of Occurences')
...
% for t = tAxisCP
%   t;
%   spCount = sum(Ry(:,:,t-width+1:t),3,'omitnan'); % Sum spike count across time, amount of time summed over defined by => width
%   for n = 1:nNeurons
%     CP(n,t) = rocN(spCount(chooseright & RT >= minRT,n), spCount(chooseleft & RT >= minRT,n), 50);
%   end
% end
% figure;plot(tAxisCP,mean(CP(rightpool,tAxisCP)),'b-','LineWidth',1.5);
% hold on;
% plot(tAxisCP,mean(CP(leftpool,tAxisCP)),'r-','LineWidth',1.5);
% xlim([width tAxisCP(end)]);
% xlabel('time (ms)');
% ylabel('CP (referenced to rightward)');
% changeAxesFontSize(gca,20,20);
% toc
