...
psth_norm_alignDotsOn = nan(length(dataCell),10,4,2000);
psth_norm_alignRT = psth_norm_alignDotsOn;

for n = 11:length(dataCell)
    n
    %*****************************
    % TUNING (map)
    %*****************************
    
    nchans = length(dataCell{n}.Mapping.Channel);
    nTrials = length(dataCell{n}.Mapping.openEvents.motionStart);
...
%             b = round(trialLen*1000)-(extRaster(2)-latency(2));
    ...
    dataCell{n}.Mapping.Selectivity_fit = nan(1,nchans);
    for c = 1:nchans
        if ~isnan(dataCell{n}.Mapping.SpikeTimes{c})
            ...
            tuning_vonMises_err = @(b,dir,FR) sum((tuning_vonMises(b,dir)-FR).^2); % sum squared error
...
%             pause
            dataCell{n}.Mapping.Selectivity_fit(c) = beta(3);
        end
    end
...
%         title(sprintf('sess %d, chan %d',n,dataCell{n}.Mapping.Channel(c)));
    ...
    if max(dataCell{n}.Exp.openEvents.choice)==2
    ...
    for t = 1:nTrials    
        for c = 1:length(dataCell{n}.Exp.Channel)
            ...
            if ~isempty(spikeTimes)
                ...
                spkInd2(spkInd2==0)=1; % can't have a zero because used as indices
...
%             b = round(trialLen*1000)-(extRaster(2)-latency(2));
            ...
            I = dataCell{n}.Exp.openEvents.direction==0 & dataCell{n}.Exp.openEvents.coherence > 0.25;
            J = dataCell{n}.Exp.openEvents.direction==180 & dataCell{n}.Exp.openEvents.coherence > 0.25;
            pref = double(mean(spRate_exp{n}(c,I)) > mean(spRate_exp{n}(c,J)));
        end
    end

    % choice/conf-conditioned PSTHs
    J = abs(dataCell{n}.Exp.openEvents.coherence)<.06; % weak motion only, or else there's a confound! can also use residuals...
    if plotflag; figure(n*10); set(gcf, 'Color', 'w', 'Position', [200 300 370*length(dataCell{n}.Exp.Channel) 430], 'PaperPositionMode', 'auto'); end
    ...
    for c = 1:length(dataCell{n}.Exp.Channel)
        ...
        psth_norm_alignDotsOn(n,c,1:4,1:size(psth,2)) = psth/rMax;
        ...
        % aligned RT
    if plotflag; figure(n*100); set(gcf, 'Color', 'w', 'Position', [200 100 370*length(dataCell{n}.Exp.Channel) 430], 'PaperPositionMode', 'auto'); end
    for c = 1:length(dataCell{n}.Exp.Channel)
        clear psth
        I = J & dataCell{n}.Exp.openEvents.choice==pref & dataCell{n}.Exp.openEvents.pdw==1; % pref-high
        ...
        psth_norm_alignRT(n,c,1:4,1:size(psth,2)) = psth/rMax;
