...
% Using matlab's random number generators. Fortunately, matlab has a number
% of tools for generating random numbers and plotting probability
% distributions. You must have the statistics toolbox. Try typing 'help
% stats' at the command prompt to see what's available.
% There are three types of functions that we will deal with. The ones
% ending in ...rnd.m generate random numbers. 
%
...
%% Expectation
% The expectation of a random variable is what is, effectively, the mean. You
% can think of it as the mean that would be calculated from all samples, where
% all might refer to an exahaustive sampling of all observations -- usually
% taken as an infinite set of observations. 
% 
...
% Notice that the deviations are smaller on average for the sample means. (Note,
% this will fail in the special case that mu is an integer.) This is not a proof
% that the sample mean is the most efficient statistic for estimating the E[X],
% but it gives you a feel for what we're talking about. 
%
% Now, let's return to the point. Why the 1/(n-1) instead of 1/n in the formula
% for the sample variance? It's because we want an estimate of the variance
% from our n samples that is unbiased and maximally efficient. if we happen to
% know mu, then taking the average of the n terms, (X-mu)^2, would be an
% unbiased estimator of E[(X-mu)^2]. But we don't know mu from our n samples.
% All we know is some estimate of mu, like the sample mean. But the sample mean
% is computed from the n samples. This is kind of a cheat. The sample mean is
% the value we would choose that is nearest all the samples, on average. The
% actual expectation, which is similar to but not exactly the same as the sample
% mean, is going to be further away from the samples, on average. This means
% that 
var1 = sum((x-sampleMean).^2) / n
% is a biased estimator of the true variance. It is biased to be smaller than
% the true variance:
% E[var1] < E[(X-mu)^2]
% We can see that from the simulated samples we took. Matlab will compute this
% biased version of the variance if you pass a second argument var(x,1)
var(x,1)
% If we use the standard matlab var(x) we get back the 1/(n-1) version of the
% sample variance. Confirm this:
sum((x-sampleMean).^2) / (n-1)
var(x)
...
% unbiased and maximally efficient. But you can make nreps bigger to convince
% yourself that it's on track. 

% So we know the definition of the mean and variance of a random variable and we
% know how to estimate the mean and variance from a finite sample of
% observations. Just to be clear, the mean of the RV is the RV's expectation,
% which is also known as the 1st moment. The variance of the RV is the 2nd
% central moment, that is the expectation of the square of the quantity, RV
% minus its expectation. The statistics we compute from samples are unbiased,
% maximally efficient estimators of these expectations. 

% The standard deviation of a random variable is the square root of its
% variance. We could state that in 4 or 5 ways using all the terminology for the
% variance, but let's get a feel for standard deviation instead. This is the
% statistic that conforms best to the spread of the distribution. It's the
% square root of a squared difference between a RV and its mean. So it's a lot like taking an absolute
% value. You should think for a second about why sqrt(var(x)) is not the same
% thing as mean(abs(x-mu))
% Matlabs
std(x)
% is the same as
sqrt(var(x))
...
% centimeters). The basic characteristization of the spread in the measurements
% should not be affected by this. And that is captured by thinking about the
% observations in terms of distance from the mean in units of standard
% deviation. Importantly, muliplying the random values by a constant multiplies
% the mean and the standard deviation by this same amount. The ratio,
% standard deviation divided by the mean, gives us an indication of the spread
% of the distribution, regardless of what units we use to describe the
% observations. This ratio is called the coefficient of variation (CV) and its
% reciprocal is sometimes called the signal to noise ratio. Changing the units,
% or turning up the amplifier, or increasing the power on the microscope, etc.
% do not change the dispersion of the data, just the units we use to describe
% the data. 
%
% Notice that the CV does not change. But, since the variance is the square of
% the stdev, it follows that another ratio, the variance divided by the mean,
% does change when we scale units differently. We'll discuss variance/mean in
% the stochastic processes tutorial. It's worth keeping this point in mind or
% coming back to it when we encounter this ratio.
...
% single sample mean. The yellow shows the sample means from nreps repetitions
% of this experiment. It's pretty obvious that the sample means fall near the
% true mean. And, as we noted earlier, the spread in the sample means about the
% true mean is a lot less than the spread of the observations in any one
% experiment. The standard error of the mean is simply the standard deviation of
% these sample means.    
stdOfSampleMeans = std(sampleMeanYs)
% Compare this to the standard deviation from the first sample of observations
std(Y(:,1))
% This set of observations is not off the mark. Remember, we used a Poisson distribution whose mean = 10.
% So the variance should also be 10 and the standard deviation should be 
sqrt(mu)
...
n = size(Y,1)
std(Y(:,1)) / sqrt(n)
% It's very easy to derive this expression. It's based solely on two points.
% First, if you make a new random variable by adding two random numbers, A + B, the
% variance of the sum is the sum of the variances associated with A and B. This
% holds if A and B are independent, that is the joint probability that A=x and B=y is
% the product of the probability that A=x (regardless of B) times the probability
% that B=y (regardless of A). The second principle is that the sample variance
% is an unbiased estimator of the true variance. We've already made that point. 
% It follows that the sum of n variables, each with mean mu and variance s^2, has
% E[sum] = n*mu
% and
% Var[sum] = n * s^2
% Stdev[sum] = sqrt(n) * s
...
% Excercise (optional): Graph three distributions, Poisson, Normal (aka, Gaussian),
% and Uniform. Do this so that they each have the identical mean = 9 and the
% same standard deviation = 3. How do these distributions differ from each
% other. [Hint: skewness & kurtosis].
