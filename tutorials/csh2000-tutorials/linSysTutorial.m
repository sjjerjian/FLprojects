...
new_impulse_response = cconv(impulse, filter,length(impulse));
...
impulse_response_10 = cconv(impulse_10,filter,length(impulse_10));
plot(nRange,impulse_response_10,'g-',nRange,impulse_response_10,'go');
axis([0 31 -.06 .12]);

impulse_response_15 = cconv(impulse_15,filter,length(impulse_15));
plot(nRange,impulse_response_15,'g-',nRange,impulse_response_15,'go');
axis([0 31 -.06 .12]);

% Filter response to sum of impulses:
signal_response = cconv(signal,filter,length(signal));
...
step_response = cconv(step,filter,length(step));
...
for n = 17:32
  ...
  sum_of_impulse_responses = sum_of_impulse_responses + ...
      cconv(impulse_n,filter,length(impulse_n));
end
plot(nRange,sum_of_impulse_responses,'r-',nRange,sum_of_impulse_responses,'go');
axis([0 31 -.1 .1]);
...
% Note that cconv performs circular convolution.  Circular
...
% values (at both ends) rather than wrapping.
...
response_1_2 = cconv(cconv(step,filter1,length(step)),filter2,length(step));
plot(nRange,response_1_2,'r-',nRange,response_1_2,'go');
axis([0 31 -.06 .06]);

response_2_1 = cconv(cconv(step,filter2,length(step)),filter1,length(step));
...
response1 = cconv(step,filter1,length(step));
response2 = cconv(step,filter2,length(step));
...
response_to_sum_of_filters = cconv(step,sum_of_filters,length(step));
...
shifted_sinusoid = cconv(sinusoid,shift_filter,length(sinusoid));
plot(nRange,shifted_sinusoid,'r-',nRange,shifted_sinusoid,'go');
axis([0 31 -1.2 1.2]);

unshifted_sinusoid = cconv(shifted_sinusoid,unshift_filter,length(shifted_sinusoid));
...
lo_impulse_response = cconv(impulse,lo_filter,length(impulse));
plot(nRange,lo_impulse_response,'r-',nRange,lo_impulse_response,'go');
axis([0 31 -.02 .32]);

hi_impulse_response = cconv(impulse,hi_filter,length(impulse));
...
lo_sin_response = cconv(sinusoid,lo_filter,length(sinusoid));
plot(nRange,lo_sin_response,'r-',nRange,lo_sin_response,'go');
axis([0 31 -.65 .65]);

hi_sin_response = cconv(sinusoid,hi_filter,length(sinusoid));
...
filter = [-.004 -.031 -.047 .031 .102 .031 -.047 -.031 -.004];
result = cconv(sinusoid,filter,length(sinusoid));
...
result = cconv(sinusoid,shift_filter,length(sinusoid));
plot(nRange,result,'r-',nRange,result,'go');
axis([0 31 -1.2 1.2]);
...
for index = 1:10
  filter = rand(1,5);
  results(index,:) = cconv(sinusoid,filter,length(sinusoid));
end
...
imag_conv_of_fts = cconv(real(ft_gaussian),imag(ft_sinusoid),length(ft_gaussian))/32;
...
impulse_response = cconv(impulse_signal,gabor_filter,length(impulse_signal));
plot([0:127], impulse_response, 'r-');
axis([0 127 -1.0 1.0]);

filtered_signal = cconv(random_signal,gabor_filter,length(random_signal));
...
delay_filter = [0 0 1];
impulse_response = cconv(impulse_signal, delay_filter,length(impulse_signal));
